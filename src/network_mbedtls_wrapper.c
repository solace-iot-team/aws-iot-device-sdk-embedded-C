...
void _iot_tls_set_connect_params(Network *pNetwork, char *pRootCALocation, char *pDeviceCertLocation,
	...
	pNetwork->tlsConnectParams.pDeviceCertPayload = pDeviceCertPayload;
	pNetwork->tlsConnectParams.deviceCertPayloadLength = deviceCertPayloadLength;
	pNetwork->tlsConnectParams.pDevicePrivateKeyLocation = pDevicePrivateKeyLocation;
	pNetwork->tlsConnectParams.pDevicePrivateKeyPayload = pDevicePrivateKeyPayload;
	pNetwork->tlsConnectParams.devicePrivateKeyPayloadLength = devicePrivateKeyPayloadLength;

	pNetwork->tlsConnectParams.pRootCALocation = pRootCALocation;
	pNetwork->tlsConnectParams.pRootCAPayload = pRootCAPayload;
	pNetwork->tlsConnectParams.rootCAPayloadLength = rootCAPayloadLength;
...
IoT_Error_t iot_tls_init(Network *pNetwork, char *pRootCALocation, char *pDeviceCertLocation,
	...
	_iot_tls_set_connect_params(pNetwork, pRootCALocation, pDeviceCertLocation, pDevicePrivateKeyLocation,
			pRootCAPayload, rootCAPayloadLength,
			pDeviceCertPayload, deviceCertPayloadLength,
			pDevicePrivateKeyPayload, devicePrivateKeyPayloadLength,
...
IoT_Error_t iot_tls_connect(Network *pNetwork, TLSConnectParams *params) {
	...
	if(NULL != params) {
		_iot_tls_set_connect_params(pNetwork, params->pRootCALocation, params->pDeviceCertLocation,
									...
									params->pRootCAPayload, params->rootCAPayloadLength,
									params->pDeviceCertPayload, params->deviceCertPayloadLength,
									params->pDevicePrivateKeyPayload, params->devicePrivateKeyPayloadLength,
	...
	if(ret < 0) {
		ret = mbedtls_x509_crt_parse(&(tlsDataParams->cacert), pNetwork->tlsConnectParams.pRootCAPayload, pNetwork->tlsConnectParams.rootCAPayloadLength );
	}
	if(ret < 0) {
	...
	if(ret < 0) {
		ret = mbedtls_x509_crt_parse(&(tlsDataParams->clicert), pNetwork->tlsConnectParams.pDeviceCertPayload, pNetwork->tlsConnectParams.deviceCertPayloadLength );
	}
	...
	if(ret != 0) {
		char* pw = "";
		ret = mbedtls_pk_parse_key(&(tlsDataParams->pkey), pNetwork->tlsConnectParams.pDevicePrivateKeyPayload, pNetwork->tlsConnectParams.devicePrivateKeyPayloadLength,(unsigned char*)pw, 0);
	}
	if(ret != 0) {
